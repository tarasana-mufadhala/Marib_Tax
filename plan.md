...
| 2026-08-28 | Antigravity | جلسة المكلف تنتهي كل ساعة | `/auth/login` كان يُسلّم رمز وصول Supabase وحده (عمره ساعة) بلا رمز تجديد، فيُخرَج المكلف من التطبيق كل ساعة في منتصف طلبه — وهذا هو مصدر «انتهت جلستك» المتكرر لا صياغة الرسالة. نُفِّذت `POST /api/v1/auth/refresh` (كانت **موصوفة في العقد مسبقاً** عند `packages/contracts/openapi/marib-tax.v1.yaml:380` وغير منفَّذة)، وصار الرد يحمل `refreshToken` و`expiresInSeconds` و`tokenType` بأسماء `TokenPairResponse`. `ApiClient` يجدّد بصمت عند أول 401 ويعيد النداء مرة واحدة، بتجديد واحد للنداءات المتزامنة لأن GoTrue يدوّر الرمز. **لـ Kimi:** ردود `/auth/login` و`/auth/refresh` تزيد `userProfileId` على `TokenPairResponse` (سلوك قائم قبل هذا التغيير) — يلزم قرار: إضافته للمخطط أم إسقاطه من الرد. |
| 2026-08-28 | Antigravity | الدخول بالبصمة في تطبيق المكلف | `local_auth` خلف عقد `BiometricService` (وبديلٌ وهمي للاختبارات). البصمة تُثبت الشخص ثم رمز التجديد يُصدر الجلسة — لا تُخزَّن كلمة المرور إطلاقاً. حالة `AuthStatus.locked` وشاشة `LockPage` تحرسان جلسة قائمة عند فتح التطبيق. جهاز حُذفت بصماته يُطفئ التفضيل بدل أن يحبس صاحبه. **يتطلب `flutter pub get`**: أُضيفت الحزمة، وصار `MainActivity` يرث `FlutterFragmentActivity` (شرط BiometricPrompt)، مع `USE_BIOMETRIC` و`NSFaceIDUsageDescription`. |
| 2026-08-29 | Antigravity | 🔧 ملف محايد: تثبيت pnpm كان يمنع التثبيت للجميع | جذر `package.json` كان يثبّت `pnpm@11.13.0` — إصدار معطوب يرفض pnpm تشغيله («@pnpm/exe shipped without a binary»)، فيفشل `pnpm install` وكل أوامر lint/typecheck/test في المشروع كله وفي CI. جُرّبت ثلاثة مسارات تجاوز بلا تعديل الملف (`--config.manage-package-manager-versions=false`، `npm_config_*`، `COREPACK_ENABLE_STRICT=0`) وكلها رُفضت، فلا سبيل غير تعديل التثبيت. رُفع إلى `pnpm@11.24.0` بموافقة المستخدم (متوافق مع `lockfileVersion: 9.0`). **تنبيه محايد لجميع الأطراف:** غيّروا نسخة pnpm المحلية لديكم إن لزم. ملاحظة تشغيلية: pnpm 11 يخرج بـ1 على `ERR_PNPM_IGNORED_BUILDS` (سكربتا `@astryxdesign/*`)، فتُشغَّل الأوامر بـ`--config.verify-deps-before-run=false` حتى تُعتمد البناءات بـ`pnpm approve-builds`. |
| 2026-08-29 | Antigravity | ⚠️ قفل الحزم لا يُولَّد إلا بنسخة Flutter المثبَّتة في CI | إضافة `local_auth` بنسخة Flutter أحدث (3.47.2) رفعت حزمَ الاختبار التي يثبّتها الـSDK نفسه (matcher، meta، test_api…)، فصار `pubspec.lock` يشترط `dart >=3.11` بينما **Flutter 3.38.7 المثبَّت في `foundation-ci.yml` يشحن Dart 3.10.7** — أي أن `flutter pub get` كان سيفشل في CI. `local_auth 2.3.0` نفسها بريئة: تشترط `flutter >=3.16.6` فقط. أُعيد توليد القفل بـ3.38.7 فصار الفرق **إضافة محضة (40 سطراً، بلا حذف)**، وأُعيد التحقق عليها: `flutter analyze` نظيف. **قاعدة للطرفين:** وَلِّدوا `pubspec.lock` بنسخة CI لا بالأحدث لديكم. |
| 2026-08-29 | Antigravity | بناء APK عبر GitHub Actions | `.github/workflows/mobile-apk.yml` — تشغيل يدوي (`workflow_dispatch`) يأخذ `api_base_url` و`build_mode`، ويرفع الـAPK كـartifact. **يرفض أي عنوان لا يبدأ بـ`https://`** لأن أندرويد 9+ يحجب http الصريح، فنسخة بعنوان http تفشل نداءاتها كلها في يد المكلف بلا سبب ظاهر. تنبيه قائم: `signingConfig` في `android/app/build.gradle.kts` ما زال مفتاح التصحيح (TODO أصلي)، فنسخة الإصدار تصلح للتجريب الداخلي لا للنشر على Play. |
| 2026-08-30 | Antigravity | 🔴 الـ API لا يصلح لـ serverless: رموز التحقق في ذاكرة العملية | `OtpService` يحفظ الرموز في `Map` داخل العملية (`otp.service.ts:16`) **ولا يوجد جدول OTP في القاعدة إطلاقاً** — والـ`db` المحقون يُستعمل لتسجيل الأحداث فقط. على Vercel serverless يقع طلب الرمز على نسخة والتحقق على أخرى فارغة، فيرتد `verifyOtp` من أول سطر (`if (!record) return false`) ويفشل التسجيل واستعادة كلمة المرور بشكل متقطّع. حتى `DEV_OTP_CODE` محجوب خلف وجود السجلّ نفسه. العدّاد `loginLockouts` له العلّة نفسها (موصوف أصلاً بأنه best-effort). **الأثر يتجاوز النشر:** أي تشغيل بأكثر من نسخة واحدة — حتى على خادم دائم — يكسر المصادقة. الحل الجذري نقل التخزين إلى القاعدة، ويستوجب migration جديداً بملكية Kimi. |
| 2026-08-30 | Antigravity | نشر تجريبي: الواجهتان على Vercel والـAPI على خادم دائم | `apps/website/vercel.json` و`apps/admin-dashboard/vercel.json` (يبنيان `@marib-tax/contracts` أولاً لأنه مستورَد **وقت التشغيل** لا كأنواع فقط)، و`render.yaml` للـAPI والـworker. أمر الإقلاع مُختبَر فعلياً: `/health` و`/ready` يردّان سليمَين. **تحذير إعداد:** `NODE_ENV` في المخطط `development` عمداً — `DEV_OTP_CODE` يُتجاهَل في `production`، وبلا Twilio/Meta لا تصل رموز التحقق فيتعذّر إنشاء أي حساب للتجربة. تُرفع إلى production فور ربط مزوّد رسائل حقيقي. |
| 2026-08-30 | Antigravity | 🔴 `pnpm-workspace.yaml` كان يُفشل التثبيت على أي نسخة نظيفة | `allowBuilds` كان يحمل نصّاً حرفياً بدل قيمة منطقية: `'@astryxdesign/cli': set this to true or false` — تعليمة أداة تُركت كقيمة. النتيجة `ERR_PNPM_IGNORED_BUILDS` و`pnpm install --frozen-lockfile` يخرج بـ**1**، فيسقط البناء على Vercel وفي أي CI. ضُبطت إلى `false` للحزمتين: سكربت `postinstall` فيهما لا يبني شيئاً بل يطبع تذكيراً بتشغيل `astryx init`، و`dist` يصل مبنيّاً مع الحزمة. تحقُّق: `pnpm install --frozen-lockfile` صار يخرج بـ0. |
| 2026-08-30 | Antigravity | نشر تجريبي ناجح للواجهتين على Vercel | الموقع `website-r0raqzdad-zarax34.vercel.app` ولوحة التحكم `marib-tax-admin-g6z6pm92w-zarax34.vercel.app` — كلاهما READY. ثلاثة عوائق حُلّت: (1) رفع مجلد التطبيق وحده يُسقط البناء بـ«No Next.js version detected» — الحل النشر من جذر المستودع مع `rootDirectory` لكل مشروع؛ (2) `seatBlock: TEAM_ACCESS_REQUIRED` لأن Vercel ينسب النشر إلى **مؤلّف الـcommit** (`ahmedalslwy4x@`) لا إلى الحساب الداخل (`ahmedalsrori12x@`) — حُلّ مؤقتاً بالنشر من نسخة بلا `.git`، والحل الدائم إضافة البريد إلى الفريق؛ (3) أنماط `.vercelignore` تُطابق أي مقطع في المسار، فكان `supabase` يستبعد `src/themes/supabase/` ويُسقط لوحة التحكم وحدها — أُسندت مسارات الجذر بـ`/`. |
| 2026-08-31 | Antigravity | النظام كله يعمل على بيئة تجريبية | الـAPI حيّ على `marib-tax-api.onrender.com` (Render، frankfurt، خطة free) و`/health` و`/ready` يردّان، ونقطة عامة تُرجع 200 فالوصل بالقاعدة سليم. الواجهتان على Vercel مربوطتان به عبر `NEXT_PUBLIC_API_URL`. عائقان حُلّا أثناء النشر: (1) `corepack enable` يفشل على Render بـEROFS لأن `/usr/bin` للقراءة فقط — أُسقط، وRender يوفّر pnpm من `packageManager`؛ (2) مجموعة البيئة `.ENV` كانت منشأة لكن غير مربوطة بالخدمة، ونقطة الربط غير متاحة في الـAPI العام، فنُسخت متغيّراتها الستة إلى الخدمة مباشرة. **الكود يُبنى من مرآة `Zarax34/marib-tax`** لأن صلاحية Antigravity على المستودع الأصلي `admin: false` فلا يملك تفويض تطبيق Render؛ `origin` يبقى مصدر الحقيقة ويُدفَع إليه أولاً. |
| 2026-08-31 | Antigravity | 🔴 `ApiExceptionFilter` كان يبتلع كل خطأ 500 بلا تسجيل | المرشِّح يمسك أي استثناء ويردّ «حدث خطأ غير متوقع» **بلا سطر واحد في السجل**، فيستحيل تشخيص أي عطل في الإنتاج. ظهر عملياً: خمس محاولات دخول ردّت 500 ولا أثر لها في سجلّ Render. صار 5xx يُسجَّل بجانب الخادم مع `traceId` نفسه الذي يراه المستخدم (4xx متوقّعة فلا تُضجّ السجل)، وجسم الرد لم يتغيّر فلا تسريب. **وأول ما كشفه هذا التسجيل هو سبب عطل الدخول نفسه.** |
| 2026-08-31 | Antigravity | 🔴 ثلاثة أعطاب في `DATABASE_URL` منعت كل وصول للقاعدة | (1) كلمة المرور تحوي `@` غير مُرمَّز فصار في الرابط علامتا `@` وانكسر تحليله؛ (2) المضيف المباشر `db.*.supabase.co` **بلا سجل IPv4 إطلاقاً** (IPv6 فقط) وRender المجاني IPv4، فالاتصال يفشل فوراً؛ (3) بعد التصحيح ظهر `self-signed certificate in certificate chain` لأن جذر شهادة Supabase ليس في مخزن ثقة Node. الحل: ترميز كلمة المرور، ومجمّع الجلسات `aws-0-ap-southeast-1.pooler.supabase.com:5432` بمستخدم `postgres.<ref>`، و`sslmode=no-verify`. **الأثر كان صامتاً ومضلِّلاً:** `/public/legal-entities` يبتلع الخطأ ويردّ `[]` فبدا سليماً، و`recordAuthEvent` يبتلعه فلم تُسجَّل أحداث، ولم يظهر العطل إلا في مسار الدخول. **دَين للإنتاج:** `no-verify` يشفّر ولا يتحقق من الشهادة — يجب تثبيت شهادة Supabase الجذرية قبل الإنتاج. |
| 2026-09-01 | Antigravity | 🔴 سبعة روابط تطوير مثبَّتة تقود المستخدم إلى جهازه هو | خمسة في الموقع تشير إلى `http://localhost:3001/login` واثنان في اللوحة إلى `http://localhost:3002` — عناوين جهاز المطوّر. على أي نسخة منشورة يقود زر «دخول الموظفين» المكلفَ إلى `localhost` على جهازه فلا يفتح شيئاً، وكذلك زر «الموقع العام» للموظف. صارت تُقرأ من `NEXT_PUBLIC_ADMIN_URL` و`NEXT_PUBLIC_WEBSITE_URL` عبر `src/lib/app-urls.ts` في كلٍّ منهما، مع إبقاء localhost تعويضاً للتطوير وحده. تحقُّق على النسخة المنشورة: **صفر مواضع localhost** والروابط تشير إلى العنوان الحقيقي. |
| 2026-09-01 | Antigravity | 🔴 الموقع واللوحة يختفي نصّهما لمن جهازه في الوضع الليلي | غلاف `Theme` من Astryx يغلّف التطبيقين ويحقن على نفسه `color-scheme: light dark` مع `color: var(--color-text-primary)`. في الوضع الليلي تنقلب رموز Astryx إلى قيمها الداكنة فيصير لون النص الموروث فاتحاً، بينما تبقى البطاقات بيضاء بألوان Tailwind الثابتة — **فنص فاتح على أبيض = غير مرئي**. التصميم فاتح بالكامل ولا وضع ليلي مبنيّ له، فأُعلن `color-scheme: light` في `:root` **غير المُطبَّق (unlayered)** ليتغلّب على قاعدة Astryx داخل `@layer astryx-base`. تحقُّق على النسخة المنشورة: القاعدة موجودة بمحدّد `:root` خارج أي طبقة. **دَين:** تُرفع حين يُبنى وضع ليلي حقيقي، وحينها يجب توحيد ألوان الموقع مع رموز Astryx بدل تثبيتها. |
| 2026-09-01 | Antigravity | رفع الواجهتين إلى production لرابطين ثابتين | كل `vercel deploy` يُنتج رابط معاينة جديداً، فكان المستخدم يختبر نسخة قديمة بينما تُصلَح أخرى — ظهر ذلك حين بقي `localhost` مرئياً لديه بعد إصلاحه والتحقق منه. النطاقان الثابتان الآن: الموقع `website-teal-beta-20.vercel.app` واللوحة `marib-tax-admin.vercel.app`، ويتحدّثان تلقائياً مع كل نشر production. وضُبط `NEXT_PUBLIC_ADMIN_URL`/`NEXT_PUBLIC_WEBSITE_URL` عليهما بدل روابط المعاينة. حماية Vercel باقية فلا يصير الموقع علنياً. |
